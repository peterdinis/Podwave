...
export const getAllPodcasts = query({
    ...
    handler: async (ctx, args) => {
        const allPodcasts = await ctx.db.query('podcasts').order('desc').collect();

        const startIndex = args.cursor
            ? allPodcasts.findIndex(podcast => podcast._id.toString() === args.cursor) + 1
            : 0;

        const paginatedPodcasts = allPodcasts.slice(startIndex, startIndex + args.limit);
